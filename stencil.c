...
void stencil(const int nx, const int ny, float *restrict image, float *restrict tmp_image, int rank)
{
  ...
  if (rank == 0)
  {
    ...
    MPI_Sendrecv(lastRowSend, nx, MPI_FLOAT, 1/*(rank + 1*/, 0, lastRowRecv, nx, MPI_FLOAT, 0/*rank +1*/, 0, MPI_COMM_WORLD, status);
    printf("deadlock 0");
  }
  ...
  else if (rank == 1)
  {
    ...
    MPI_Sendrecv(firstRowSend, nx, MPI_FLOAT, 0/*rank - 1*/, 0, firstRowRecv, nx, MPI_FLOAT, 1/*rank*/, 0, MPI_COMM_WORLD, status);
